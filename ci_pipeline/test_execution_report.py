...
class ExecutionReportTest(unittest.TestCase):
    ...
    def test_compile_all_classifies_the_frozen_72_modules(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with (DATA / "semantic_conformance_capabilities.toml").open("rb") as stream:
            ...
            report = classify_compile_all(
                ...
                DATA / "semantic_conformance_capabilities.toml",
            )
    ...
    def test_exact_test_dis_deviations_are_lane_scoped(self):
        ...
        deviations = json.loads(
            (DATA / "execution_compatibility_deviations.json").read_text()
        )["deviations"]
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            report = compare_with_deviations(
                stock_path,
                execute_path,
                DATA / "execution_compatibility_deviations.json",
            )
    ...
    def test_matching_state_with_wrong_diagnostic_fails(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            report = compare_with_deviations(
                left, right, DATA / "execution_compatibility_deviations.json"
            )
        self.assertEqual(report["result"], "FAIL")
