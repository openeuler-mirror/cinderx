from ci_pipeline.jit311.lifecycle_snapshot import BOOLEAN_PATHS, REQUIRED_PATHS
from ci_pipeline.jit311.lifecycle_acceptance import (
...
def test_capacity_errors_become_diagnostics_not_verdicts():
    result = apply_acceptance_policy(_c1_result())
...
def test_real_errors_survive_the_demotion():
    ...
    result["result"] = "FAIL"
    judged = apply_acceptance_policy(result)
...
BROAD_PATTERNS = [
    ...
    "ci_pipeline/jit311/lifecycle_snapshot.py",
]
...
def test_trigger_ignores_unrelated_paths():
    changed = ["docs/design/whatever.md", "ci_pipeline/test_lifecycle_acceptance.py"]
...
_ALL = {"LIFECYCLE_OWNERSHIP", "SHUTDOWN_STABILITY", "NATIVE_MEMORY_SAFETY"}
_GREEN = {name: {"result": "PASS"} for name in _ALL}


def test_subset_or_skipped_regression_never_claims_a_full_pass():
    # Review P0-2.
    subset = final_status(
        selected={"LIFECYCLE_OWNERSHIP"},
        cases={"LIFECYCLE_OWNERSHIP": {"result": "PASS"}},
...
def test_full_pass_requires_every_case_and_the_regression_when_required():
    full = final_status(
        selected=set(_ALL),
        cases=dict(_GREEN),
        regression_required=True,
        regression_skipped=False,
        regression_run={"result": "PASS"},
        infra=[],
    )
    assert full == "PASS"
    ...
    failing_regression = final_status(
        selected=set(_ALL),
        cases=dict(_GREEN),
        regression_required=True,
        regression_skipped=False,
        regression_run={"result": "FAIL"},
        infra=[],
    )
    assert failing_regression == "FAIL"
...
def test_c4_phase_evidence_is_part_of_non_vacuity():
    ...
    flag_pause = apply_acceptance_policy(
        _c4_result(
            [
                {
                    "round": 100,
    ...
    parked_model = apply_acceptance_policy(
        _c4_result(
            [
                {
                    "round": 100,
    ...
    never_populated = apply_acceptance_policy(
        _c4_result(
            [
                {
                    "round": 100,
    ...
    no_death_effect = apply_acceptance_policy(
        _c4_result(
            [
                {
                    "round": 100,
    ...
    missing = apply_acceptance_policy(_c4_result([]))
