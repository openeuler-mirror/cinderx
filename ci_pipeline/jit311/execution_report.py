"""Compile-All classification, exact deviation matching, and reporting."""
...
def classify_compile_all(
    ...
    if len(targets) != 72 or len(targets) != len(set(targets)):
        raise ValueError(f"the frozen stdlib manifest must contain 72 unique modules, got {len(targets)}")
