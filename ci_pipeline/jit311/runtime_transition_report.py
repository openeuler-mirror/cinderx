...
def render_native_recursion_boundary_report(comparison: dict, out: Path) -> None:
    ...
    lines = [
        "# CPython 3.11 JIT Native Recursion Boundary Report",
        "",
...
def render_recursion_boundary_report(
    ...
    lines = [
        "# CPython 3.11 JIT Recursion Boundary Report",
        "",
...
def render_frame_position_report(
    ...
    lines = [
        "# CPython 3.11 JIT Frame Position Report",
        "",
...
def render_policy_footprint_report(result: dict, out: Path) -> None:
    ...
    lines = [
        "# CPython 3.11 JIT Transition Policy and Footprint Report",
        "",
...
def render_footprint_deviation_proof_final(
    ...
    delta = footprint.get("delta", {})
    lines = [
        "# CPython 3.11 JIT Footprint Deviation Proof Final",
        "",
...
def render_markdown(final: dict, path: Path) -> None:
    ...
    lines = [
        "# CPython 3.11 CinderX JIT Runtime-Transition Report",
        "",
        f"Final: **{final['final']}**",
        "",
        "## Autocompile coverage (aggressive penetration)",
        "",
    ...
    lines.extend(
        [
            "",
            "## Transition recovery (repetition)",
            "",
    ...
    lines.extend(
        [
            ...
            f"- Conclusion: **{final.get('freeze_state', 'NOT_FROZEN')}**",
            "",
            "## Blockers",
            "",
        ]
