# CPython 3.11 runtime-transition acceptance implementation

The runtime-transition acceptance is a discovery-first gate over the clean
execution-domain capability surface. It does not expand the opcode whitelist
and only the P lane forbids explicit compilation; the T lane may use the
private generator compile diagnostic because automatic generator scheduling
is deliberately disabled by execution-domain policy.
...
allocation failure increments `dropped`; the acceptance fails when it is
non-zero.

## Existing-test mapping

| Case | Existing native/Python anchor |
|---|---|
...
| T06 | tracing conformance T1-T8 |
| T07 | `test_mid_frame_pep523_takeover_does_not_capture_the_deopt` |
| T08 | generator send/throw/yield-from and the native-C boundary probe |
...
The transition probes organize those contracts into one machine-readable transition
...
- Coverage uses the execution domain's non-retaining exact per-code entry
  ledger.
- Official runs inherit the execution acceptance's clean-source and exact
  wheel/source SHA preflight.
